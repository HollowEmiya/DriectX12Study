...
void Waves::Update(float dt)
{
	...
	if (t >= mTimeStep)
	{
		...
		concurrency::parallel_for(1, mNumRows - 1, [this](int i)
			// for(int i = 1; i < mNumRows - 1; ++i)
			{
				for (int j = i; j < mNumCols - 1; ++j)
				{
					...
					// Note j indexes x and i indexed z: h(x_j, z_i, t_k)
		...
		// so this data needs to become the current solution 
		...
		concurrency::parallel_for(1, mNumRows - 1, [this](int i)
			// for(int i = 1; i < mNumRows - 1; ++i)
			{
				for (int j = 0; j < mNumCols - 1; j++)
				{
					...
					mTangentX[i * mNumCols + j] = XMFLOAT3(2.0f * mSpatialStep, r - 1, 0.0f);
