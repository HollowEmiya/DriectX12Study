...
private:
	...
	float mTheta = 1.3 * XM_PI;		// theta on horizontal x,z
	float mPhi = 0.4 * XM_PI;		// phi on vertical y
	float mRadius = 2.5f;
...
void CrateApp::UpdateMaterialCBs(const GameTimer& gt)
{
	...
	for (auto& e : mMaterials)
	{
		...
		{
			XMMATRIX matTrasform = XMLoadFloat4x4(&mat->MatTransform);

			MaterialConstants matConstants;
			matConstants.DiffuseAlbedo = mat->DiffuseAlbedo;
			matConstants.FresnelR0 = mat->FresnelR0;
			matConstants.Roughness = mat->Roughness;
			XMStoreFloat4x4(&matConstants.MatTransform, XMMatrixTranspose(matTrasform));
